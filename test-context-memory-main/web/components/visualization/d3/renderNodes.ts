  ...
  container.selectAll<SVGGElement, MemoryNode>(".memory-bubble")
    .each(function(d) {
      ...
      d3.select(this)
        ...
        .transition()
        .duration(300)
        .attr("stroke", isSelected ? "#0f172a" : (isConnected ? "#10b981" : "#ffffff"))
        .attr("stroke-width", isSelected ? 5 : (isConnected ? 3.5 : 2))
        .attr("stroke-opacity", isSelected || isConnected ? 1 : 0.85)
        .style("filter", isSelected 
          ? "drop-shadow(0px 0px 8px rgba(16, 185, 129, 0.85))" 
          : (isConnected ? "drop-shadow(0px 0px 5px rgba(245, 158, 11, 0.7))" : "none"))
