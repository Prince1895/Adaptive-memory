...
import { ChevronDown, Brain, Loader2, ArrowUp, Sparkles } from "lucide-react";
import type { RelevantMemoryItem } from "@/types/memory";

function RetrievedMemoriesRanking({ memories }: { memories: RelevantMemoryItem[] }) {
  ...
  return (
    <div className="mt-1.5 w-full max-w-[85%] text-xs">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center justify-between w-full px-3 py-1.5 rounded-lg bg-muted/40 hover:bg-muted/70 border border-border/50 text-muted-foreground hover:text-foreground transition-all"
      >
        <span className="flex items-center gap-1.5 font-medium">
          <Brain className="w-3.5 h-3.5 text-accent" />
          <span>Used {memories.length} Memories</span>
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-accent/15 text-accent font-semibold">
            Top Relevance: {(topMatch.score * 100).toFixed(0)}%
          </span>
        </span>
      ...
      {isOpen && (
        <div className="mt-1.5 p-3 rounded-xl bg-card border border-border shadow-md space-y-2.5">
          <div className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider flex items-center justify-between">
            <span>MMR Priority Ranking</span>
            <span>Relevance & Importance</span>
          </div>

          <div className="space-y-2">
            {memories.map((mem, idx) => {
              const importancePct = Math.round((mem.importance || 0.8) * 100);
              const relevancePct = Math.round((mem.score || 0.88) * 100);

              return (
                <div key={idx} className="p-2 rounded-lg bg-muted/30 border border-border/40 text-xs">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-2 py-0.5 rounded-full text-[10px] bg-amber-500/15 text-amber-600 font-medium">
                        Importance: {importancePct}%
                      </span>
                    </div>
                  </div>
                  <p className="text-foreground/90 font-normal leading-snug">
                    "{mem.memory}"
                  </p>
                </div>
              );
          ...
          <>
            {messages.map((message, idx) => (
              ...
              >
                ...
                {/* Used Memories & MMR Priority Ranking */}
                {message.role === "assistant" && message.relevantMemories && message.relevantMemories.length > 0 && (
                  <RetrievedMemoriesRanking memories={message.relevantMemories} />
                )}
              </div>
            ))}
