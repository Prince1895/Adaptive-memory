...
export class ContextMemoryAPI {
  ...
  async getMemories(): Promise<MemoriesResponse> {
    try {
      const response = await fetch(`${this.baseUrl}/api/memories/graph`, {
        headers: this.getAuthHeaders(),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          if (refreshed) {
            ...
            const retryResponse = await fetch(`${this.baseUrl}/api/memories/graph`, {
              headers: this.getAuthHeaders(),
            });
            if (retryResponse.ok) {
              const json = await retryResponse.json();
              return Array.isArray(json) ? { nodes: json as any, links: [] } : (json || { nodes: [], links: [] });
            }
          }
        }
        return { nodes: [], links: [] };
  ...
  async updateMemory(memoryId: number, text: string): Promise<void> {
  ...
  async consolidateMemories(conversationId?: number): Promise<{ consolidated_count: number }> {
    const response = await fetch(`${this.baseUrl}/api/memories/consolidate`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...this.getAuthHeaders(),
  ...
  async purgeMemories(days = 30): Promise<{ purged_count: number }> {
    const response = await fetch(`${this.baseUrl}/api/memories/purge`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...this.getAuthHeaders(),
    ...
    if (!response.ok) {
      throw new Error(`Failed to purge memories: ${response.statusText}`);
    }

    return response.json();
  }

  async searchMemories(query: string, conversationId?: number, limit = 10): Promise<any[]> {
    const response = await fetch(`${this.baseUrl}/api/memories/search`, {
      ...
      body: JSON.stringify({ query, conversation_id: conversationId, limit }),
    });
